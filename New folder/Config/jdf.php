...
/*	F	*/
function jFulltdate($timestamp='',$none='',$timezone='Asia/Tehran',$tn='en'){
	$ts=($timestamp=='')?time():tr_num($timestamp);
	$jdate=explode('_',jdate('F_G_i_j_l_n_s_w_Y_z',$ts,'',$timezone,$tn));

	return  $jdate[8]."-".$jdate[5]."-".$jdate[3]."#".$jdate[1].":".tr_num((int)tr_num($jdate[2]),$tn).":".tr_num((int)tr_num($jdate[6]),$tn);
   }
